...
router.route("/").get((req, res, next) => {
  ...
  let fetchedData = Organization.find(queryObject)
    .select("-password")
    .populate("org_projects", "title description uid thumbnail");

  // if user has written `?sort=name,domain` with multiple sort conditions in URL :
  if (sort) {
    // FOR SORTING BASED ON ANY KEY
  ...
  fetchedData
    ...
    .catch((error) =>
      next(new ApiError(422, "Error fetching organizations.", error.toString()))
    );
});
...
router
  ...
  .post(
    body("password")
      ...
      .withMessage("Password should be atleast 8 and maximum 16 characters"),
    async (req, res, next) => {
      ...
      if (!password) {
        ...
        return next(
          new ApiError(
            400,
            "Organization registration failed. Please provide a password.",
            `${path} : ${msg}`
          )
        );
      }
      ...
      if (!errors.isEmpty()) {
        ...
        return next(
          new ApiError(
            400,
            "Organization registration failed. Please check your inputs.",
            `${path} : ${msg}`
          )
        );
      }
      try {
        ...
        return controller.postOrganization(
          res,
          next,
          securedOrganization,
          file
        ...
        return next(
          new ApiError(422, "Error creating organization..", error.toString())
        );
      }
    }
  );
...
router.route("/auth/login").post((req, res, next) => {
  ...
  Organization.findOne({ uid: req.body.uid })
    ...
    .catch((error) => {
      next(
        new ApiError(422, "Error logging in Organization.", error.toString())
      );
    });
});
...
router
  ...
  .get((req, res, next) => {
    Organization.findOne({ uid: req.params.uid })
      ...
      .catch((error) =>
        next(
          new ApiError(422, "Error fetching organization.", error.toString())
        )
      );
  })
  ...
  .patch(isOrganizationAuthenticated, (req, res, next) => {
    ...
    try {
      ...
      next(
        new ApiError(422, "Error updating organization..", error.toString())
      );
    }
  })
  ...
  .delete(isOrganizationAuthenticated, (req, res, next) => {
    Organization.deleteOne({ uid: req.params.uid })
      ...
      .catch((error) =>
        next(
          new ApiError(400, "Error deleting organization.", error.toString())
        )
      );
  });
